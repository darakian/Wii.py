...
class U8(WiiArchive):
    ...
    def _load(self, data):
        ...
        for node in nodes:
            counter += 1
            name = strings[node.name_offset:].split(b'\0', 1)[0]
...
class CCF:
    ...
    def compress(self, folder):
        ...
        fileHdr.magic = b"\x43\x43\x46\x00"
        fileHdr.zeroes12 = b'\x00' * 12
        fileHdr.rootOffset = 0x20
        fileHdr.zeroes8 = b'\x00' * 8
        ...
        self.fd.write(b'\x00' * (fileHdr.filesCount * len(self.CCFFile())))
    ...
    def decompress(self):
        ...
        if fileHdr.magic != b"\x43\x43\x46\x00":
        ...
        for x in range(fileHdr.filesCount):
            ...
            output = open(fileEntry.fileName.rstrip(b'\0'), 'w+b')
